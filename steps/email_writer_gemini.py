"""Generate four personalised outreach emails per contact via Gemini."""
    ...
    for attempt in range(4):
        try:
            ...
            if "429" in error_str or "quota" in error_str.lower() or "rate" in error_str.lower():
                ...
                logger.error("Row %s Gemini API error: %s", idx, exc)
                return idx, None, error_str
...
async def write_emails(df: pd.DataFrame, notifier: Callable, save_path: str = None) -> pd.DataFrame:
    ...
    if "Pipeline_Error" not in df.columns:
        ...
        df["Pipeline_Error"] = df["Pipeline_Error"].astype(str).fillna("")
    ...
    await notifier(f"✍️ Generating outreach emails for {total} contacts via Gemini…")
